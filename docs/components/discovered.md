# All components

Documentation is generated from private Cognityx repositories. Source
links require authorization; use the access-request link if GitHub returns
a not-found page.

## Cognityx Core

Shared backend contracts, configuration, factories, and publication boundaries.

- [Open documentation](/core/)
- [Source repository (access required)](https://github.com/cognityx/cognityx-core)
- [Request repository access](mailto:bhujay.bhatta@yahoo.com?subject=Access%20request%20for%20cognityx/cognityx-core)

## Cognityx Inference

Inference serving, model lifecycle, and hardware-boundary evaluation. The repository is currently named llm-benchmark.

- [Open documentation](/inference/)
- [Source repository (access required)](https://github.com/cognityx/llm-benchmark)
- [Request repository access](mailto:bhujay.bhatta@yahoo.com?subject=Access%20request%20for%20cognityx/llm-benchmark)

## Cognityx Training

Training workflows, configuration, evaluation, and tuning.

- [Open documentation](/training/)
- [Source repository (access required)](https://github.com/cognityx/cognityx-training)
- [Request repository access](mailto:bhujay.bhatta@yahoo.com?subject=Access%20request%20for%20cognityx/cognityx-training)

## Cognityx Storage

Provider-neutral storage operations and local backend boundaries.

- [Open documentation](/storage/)
- [Source repository (access required)](https://github.com/cognityx/cognityx-storage)
- [Request repository access](mailto:bhujay.bhatta@yahoo.com?subject=Access%20request%20for%20cognityx/cognityx-storage)

## Cognityx Jobs

Durable background jobs, ownership boundaries, cancellation, and replayable progress events.

- [Open documentation](/jobs/)
- [Source repository (access required)](https://github.com/cognityx/cognityx-jobs)
- [Request repository access](mailto:bhujay.bhatta@yahoo.com?subject=Access%20request%20for%20cognityx/cognityx-jobs)
